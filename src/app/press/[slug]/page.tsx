...
export default async function NewsPostPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      <article className="story-article pt-24 sm:pt-32">
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          <Link href="/press" className="route-link">
            ← Newsroom
          </Link>
          <p className="eyebrow mt-8 text-gold">{post.category}</p>
          <h1 className="display-xl mt-5 text-[10vw] leading-[0.9] sm:text-5xl">
            {post.title}
          </h1>
        ...
        {post.image ? (
          <div className="story-article__image relative mx-auto mt-10 aspect-16/9 w-full max-w-6xl overflow-hidden border-y border-bone/12">
            <OptimizedImage
              src={post.image}
              alt={post.title}
              fill
        ...
        <div className="story-article__body mx-auto max-w-3xl px-5 py-14 sm:px-8">
          ...
          {post.sourceUrl ? (
            ...
            >
              <span className="button__label">
                Voir la source : {post.source ?? "l'article"}
              </span>
          ...
          <div className="story-article__related mt-16 border-t border-bone/12 pt-8">
              ...
              {others.map((item) => (
                <li key={item.slug}>
                  <Link
                    ...
                    className="editorial-row group flex items-baseline justify-between gap-6 py-4"
                  >
