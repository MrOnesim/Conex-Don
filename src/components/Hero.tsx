...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const p = Math.min(1.2, Math.max(0, window.scrollY / vhRef.current));
      ticker++;
      ...
      if (contentRef.current) {
        contentRef.current.style.opacity = String(Math.max(0, 1 - Math.max(0, p - 0.45) * 2.2));
      }
      yearsRef.current.forEach((node, index) => {
        if (!node) return;
        const delay = index * 0.16;
        const local = Math.min(1, Math.max(0, (p - 0.18 - delay) / 0.28));
  ...
  return (
    ...
    >
      <div ref={contentRef} className="mx-auto w-full max-w-[1600px] px-5 sm:px-8 lg:px-12">
        <h1 className="display-xl flex flex-wrap items-center text-[clamp(2rem,19vw,4rem)] leading-[0.78] sm:text-[clamp(1.5rem,16vw,3rem)] lg:text-[clamp(3rem,15rem,5rem)]">
          ...
          <span
            ...
            style={{ fontFamily: "var(--font-serif-display)", textTransform: "none" }}
          >
            &amp;
          </span>
      ...
      <div className="relative">
          ...
          {years.map((year, index) => (
            <span
              ...
              className={`display-xl text-2xl sm:text-4xl ${
                index === years.length - 1 ? "text-gold" : "text-bone/35"
              }`}
        ...
        <div className="mx-auto mt-8 max-w-[1600px] border-t border-bone/20 px-5 pb-10 sm:px-8 lg:px-12">
          <div className="flex flex-col gap-6 pt-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-md">
              <p className="eyebrow text-bone/45">Dernière sortie — 7 mai 2026</p>
              <p className="display-xl mt-3 text-2xl sm:text-3xl">DESSIGUIMANZANBERA</p>
